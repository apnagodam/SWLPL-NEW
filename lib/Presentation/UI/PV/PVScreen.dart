...
import 'package:emp_apnagodam/Domain/PV/PvService.dart';
...
class Pvscreen extends ConsumerStatefulWidget {
  const Pvscreen({super.key});

  @override
  ConsumerState<Pvscreen> createState() => _PvscreenState();
}

class _PvscreenState extends ConsumerState<Pvscreen> {
    ...
    return Scaffold(
      body: Platform.isIOS
          ? CupertinoPageScaffold(
              navigationBar: cupertinoAppBar("Update PV", context),
              child: pvLayout(context, ref))
          : Scaffold(
              appBar: AppBar(
  ...
  pvLayout(BuildContext context, WidgetRef ref) => Padding(
        ...
        child: ListView(
          children: [
            ref.watch(pvTerminalsProvider).when(
                          ...
                          .watch(postPvProvider(
                                  pvModel: Pvmodel(
                                      ...
                                      stackNo: double.parse(
                                          "${ref.watch(stackProvider)?.stackNo}"),
                                      commodityId: '',
  ...
  var terminalProvider = StateProvider<Datum?>((ref) => null);
...
enum PlusMinusType {
  ...
  const PlusMinusType(this.type, this.title);
...
extension parseInt on String? {
  toInt({int defaultValue = 0}) => int.parse(this ?? defaultValue.toString());
}
