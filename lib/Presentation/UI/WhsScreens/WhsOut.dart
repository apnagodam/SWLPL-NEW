...
class WhsOutScreen extends ConsumerStatefulWidget {
  const WhsOutScreen({super.key});

  @override
  ConsumerState<ConsumerStatefulWidget> createState() => _GuardinscreenState();
}

class _GuardinscreenState extends ConsumerState<WhsOutScreen> {
        ...
        body: SingleChildScrollView(
          child: Column(
            children: [
              Padding(
                padding: Pad(all: 10),
                child: ref.watch(whsOutTerminalDataProvider).when(
              ...
              child: simpleButton(
                  ...
                  callback: () {
                    if (ref.watch(sealImageProvider)[index] != null &&
                      ...
                      showLoaderDialog(context);
                      ref
                          .watch(savewhsOutDataProvider(
                                  getNo: "${data.gateNo}",
                                  lockMo: "${data.lockNo}",
                                  sealNo:
                                      ref.watch(gateSealProvider)[index]?.text,
                                  remark: ref
